...
export default function BookingForm() {
  ...
  useEffect(() => {
    ...
    if (date && barberId && serviceId) {
      fetch('/api/available-times', {
        ...
        .then((res) => res.json())
        .then((arr: unknown) =>
          Array.isArray(arr) ? setAvailableTimes(arr as string[]) : setAvailableTimes([]),
        )
  ...
  return (
    <section id="booking" className="bg-black text-white py-20 px-6">
      ...
      {/* Contenedor que divide la sección en imagen + formulario */}
      <div className="max-w-5xl mx-auto flex flex-col lg:flex-row items-center lg:items-start gap-8">
        {/* Imagen decorativa a la izquierda (oculta en móviles) */}
        <div className="hidden lg:block lg:w-5/12">
          <Image
            src="/fondo.png"
            alt="Decoración de reserva"
            width={600}
            height={600}
